...
test('changes all UI languages and metadata without renaming stored music', async ({ page }) => {
  ...
  await expect(page).toHaveTitle('Drum Machine — Dein Übungspartner')
  await expect(page.locator('meta[name="description"]')).toHaveAttribute('content', /Akustisches Schlagzeug/)
